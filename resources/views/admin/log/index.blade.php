        ...
        <div class="card-body">
            <div class="card-header py-3">
                <div class="row g-2">
                    ...
                    <div class="col-lg-3">
                        <select onchange="extrafilter()" class="form-select lead-filter" name="lead" id="validationDefault04">
                            <option  value="">All Leads</option>
                            @if ($leads)
                                @forelse ($leads as $lead)
                                    <option {{(isset($_GET['leadFilter'])) ? ($lead->id == $_GET['leadFilter'] ? 'selected' : '') : ""}} value="{{$lead->id}}" >{{$lead->lead_id}}
                                    </option>
                                @empty
                    ...
                    <div class="col-lg-4 d-flex flex-row">
                        ...
                        <input type="text" class="form-control date-range btn btn-outline-secondary" name="logs-date-filter"
                        id="logs-date-filter" value="" onchange="extrafilter(event);" />
