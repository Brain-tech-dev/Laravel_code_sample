        ...
        <tbody>
            ...
            @forelse ($clients as $key => $item)
                <tr class="{{($item->is_test == 1 && admin()->user()->role_id == 'superadmin') ? 'table-warning' : ''}}">
                    ...
                    <td>
                        <a href="{{ route('admin.client.client-bids-list', ['id' => $item->id]) }}" class="text-success" data-bs-toggle="tooltip" data-bs-placement="bottom" title="View" rel="facebox">
                            {{ $item->bids->count()}}
                        </a>
                    </td>
                    <td>{{ $item->mobile }}</td>
                    ...
                    <td>
                        <div class="table-actions d-flex align-items-center gap-3 fs-6">
                            <a href="{{ route('admin.client.edit', ['id' => $item->id]) }}" class="text-warning"
                                data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit"><i class="fa-solid fa-pen-to-square text-warning"></i></a>
                            <a href="javascript:void(0);"
                                onclick="deleteItem('{{ route('admin.client.delete', ['id' => $item->id]) }}')"
